...
void mlir::torch::onnx_c::populateDefaultDomainGtoP(
  ...
  patterns.onOp(
      "Gemm", 1, [](OpBinder binder, ConversionPatternRewriter &rewriter) {
        ...
        SmallVector<int64_t> custom_int_list_attr;
        ElementsAttr custom_int_tensor_attr;
        if (binder.f32FloatAttr(custom_float_attr, "custom_float_attr", 1.0f) ||
            binder.s64IntegerAttr(custom_int_attr, "custom_int_attr", 0) ||
            binder.f32FloatArrayAttr(custom_float_list_attr, "custom_float_list_attr", {}) ||
            binder.s64IntegerArrayAttr(custom_int_list_attr, "custom_int_list_attr", {})) {
              return failure();
        }
        if (custom_float_list_attr.size() > 0 && custom_int_list_attr.size() > 0) {
            auto loc = binder.getLoc();
            if (binder.denseElementsAttr(custom_int_tensor_attr, "custom_int_tensor_attr")) {
              return failure();
            }
            
            auto tensorType = llvm::dyn_cast<mlir::RankedTensorType>(custom_int_tensor_attr.getType());
            auto torchTensorType = mlir::torch::Torch::ValueTensorType::get(
            rewriter.getContext(), tensorType.getShape(), tensorType.getElementType());
            Value cstLut = rewriter.create<Torch::ValueTensorLiteralOp>(loc, torchTensorType, custom_int_tensor_attr).getResult();

            //Value cstCustomFloat = rewriter.create<Torch::ConstantFloatOp>(loc, rewriter.getF32FloatAttr(custom_float_attr));
            Value cstCustomInt = rewriter.create<Torch::ConstantIntOp>(loc, rewriter.getI64IntegerAttr(custom_int_attr));
            //Value cstCustomFloatList = createConstantFloatList(binder, rewriter, custom_float_list_attr);
            Value cstCustomIntList = createConstantIntList(binder, rewriter, custom_int_list_attr);  
            Value bindA; 
            auto  bindAType = dyn_cast<Torch::ValueTensorType>(a.getType());
            if (!bindAType) {
              return rewriter.notifyMatchFailure(binder.op, "Expected tensor type for operand a");
            }
